         subroutine chinn5(iclear)
c
c
c        chiral nn potential in momentum space.
c        this code contains all expressions up to N3LO
c        as obtained from dimensional regularization (DR)
c        as well as spectra-function regularization (SFR).
c        To invoke the SFR expressions, the third parameter on
c        the first input paramter line (the "factor typ" line) 
c        must be non-zero. This non-zero parameter is the SFR cutoff.
c        If this parameter is zero, the DR expressions are used.
c        (Note that the second parameter of this line is useless.)
c
c
...
c
      IF (iclear==.true.) then
         index = .false.
         write(kwrite,"(a)") 'reset chinn5 index'
         return
      END IF
...
c       inversion
c
      call dminv_chinn5 (adminv,4,deter,ldminv,mdminv)
c
c
c
c
...
c
  ...
  815 call chistr_chinn5(1,mx,mx)
c
c
c
c
...
c**** vv0(1)=clsj(1,1)*fff/fac
c*** 1 /(1.d2*(2.d0*pi)**3)
c**** vv2(1)=(xx+yy)*clsj(2,1)*fff/fac
c****1 /(dwn**2*1.d8*(2.d0*pi)**3)
c**** vv4(1)=((xx*xx+yy*yy)*clsj(3,1)
c****1      +xy*xy*clsj(4,1))*fff/fac
...
c        ---
c*****************************
c        3p0 p.w. pot calculated directly
c**** vv0(3)=0.d0
c**** vv2(3)=xy*clsj(1,2)*fff/fac
c****1 /(dwn**2*1.d8*(2.d0*pi)**3)
c**** vv4(3)=(xx+yy)*xy*clsj(2,2)*fff/fac
...
c**** vv0(4)=0.d0
c**** vv2(4)=xy*clsj(1,10)*fff/fac
c****1 /(dwn**2*1.d8*(2.d0*pi)**3)
c**** vv4(4)=(xx+yy)*xy*clsj(2,10)*fff/fac
c****1 /(dwn**4*1.d14*(2.d0*pi)**3)
c*****************************
c
c        Q^2 term
...
c**** vv0(4)=0.d0
c**** vv2(4)=0.d0
c**** vv4(4)=xx*yy*clsj(1,15)*fff/fac
c****1 /(dwn**4*1.d14*(2.d0*pi)**3)
c*****************************
c
      if (cutlsj(1,15).eq.0.d0) then
...
c
      logical indc,indxy,indpar
c
      common /compar/ cb1,cb2,cb3,cb4,cb5,
     1                cd12,cd3,cd5,cd145,cd18
c
      common /comlsj/ clsj(15,50),cutlsj(15,50),indlsj
      logical indlsj
c
...
c
      ...
      integer imga(3)
      integer cut,cuta,fun,lsj,lec,end
      integer mesong(40)
      logical index
      logical zerocp
      logical indlec
...
10011 format (//' chinn5: chiral NN code up to N3LO with DR and FSR expr
     1essions')
...
c
c
 4600 c(mm,im)=1.d0/(pi*(16.d0*pi*fpi3)**2)
      ic(mi+1,im)=cc(2)
      mi=mi+2
      mm=mm+1
      go to 5995
...
c
c
 4700 c(mm,im)=ga2*cb4/(pi*(16.d0*pi*fpi3)**2)
      ic(mi+1,im)=cc(2)
      mi=mi+2
      mm=mm+1
      go to 5995
...
c
c
      call dgelg_chinn5 (b,a,7,1,eps,ier)
c
...
c
c
      call dgelg_chinn5 (b,a,15,1,eps,ier)
c
...
c
 9999 stop
      ...
      subroutine chistr_chinn5 (icase,max,mex)
c
...
c
c
      call gset_chinn5 (-1.d0,1.d0,nt,ct,wt)
      nnt=nt
c
c
c
...
c
      ...
      do 99 i=1,nt
      t=ct(i)
      call legp_chinn5 (pj(1,i),pj(3,i),t,j)
      pj(2,i)=pj(1,i)*t
      pj(4,i)=pj(2,i)*t
      pj(6,i)=pj(4,i)*t
      pj(5,i)=pj(3,i)*t
...
c
c        further specifications
c**** complex*16 cterm1,cterm2,cterm3,cterm4,cterm5
      ...
      logical indla,ind445,indmu
      data iinter/-1/
      data cc4/-1.d0/,cc445/-1.d0/,cc4mu/-1.d0/
      data nnxi/-1/
      data nnmu/-1/
      data pi/3.141592653589793d0/
      save
...
c
      call gset_chinn5 (0.d0,acut,nmu,amu,wmu)
c
...
c
 4400 continue
      if (.not.indmu.or.cc4mu.ne.c4) go to 40
c
      nxi=ic(mi+1,im)
      if (nxi.ne.nnxi) then
      nnxi=nxi
      call gset_chinn5(0.d0,1.d0,nxi,xi,wxi)
      end if
c
      wpi=dsqrt(c4)
      wpi2=2.d0*wpi
...
c
      aintmua=aintmua
     1      +aintxia*wmu(ii)*root15/(amu4(ii)*(amu2(ii)+aakk))
      aintmub=aintmub
     1      +aintxib*wmu(ii)*root15/(amu4(ii)*(amu2(ii)+aakk))
 4415 continue
c
      aintmu=aintmua+aintmub
c
 4405 aa(i)=aa(i)*c5*aintmu
      mi=mi+2
      mm=mm+1
      go to 5999
c
...
c
      ...
      brak5=ga2*ga2*(akk-2.d0*c4*wbarq*xixi+2.d0*c4)
c
c
...
c
 4525 aintxi=aintxi+step45
c
c
 4515 aintmu=aintmu
     1      +aintxi*wmu(ii)*root/(amu6(ii)*(amu2(ii)+aakk))
c
c
 4505 aa(i)=aa(i)*c5*aintmu
      mi=mi+2
      mm=mm+1
      go to 5999
c
...
c
      wbarq=1.d0+akk/(4.d0*c4)
      wbar=dsqrt(-wbarq)
      brak1=cb3*(2.d0*c4+akk)-4.d0*cb1*c4
      brak2=2.d0*cb2*c4*wbarq
      wtilq=2.d0*c4+akk
      cpxx=wtilq/(4.d0*amu(ii))*dlog((amu(ii)+wpi2)/(amu(ii)-wpi2))
      brak3=3.d0*ga2*(c4+2.d0*akk)*(wpi+cpxx)
c
c
      aintxi=0.d0
      do 4625 iii=1,nxi
      xixi=xi(iii)*xi(iii)
      term1=-wbarq*c4*xixi
      term2=12.d0*term1*dsqrt(c4+term1)
      term3=4.d0*ga4*(2.d0*term1+akk+2.d0*c4)/term1
      term4=(c4+term1)**1.5d0-c4*wpi
c
c
      step46=wxi(iii)*(brak1+brak2*xixi)
     1      *(brak3+term2+term3*term4)
c
 4625 aintxi=aintxi+step46
c
c
 4615 aintmu=aintmu
     1      +aintxi*wmu(ii)*root/(amu6(ii)*(amu2(ii)+aakk))
...
c
      wbarq=1.d0+akk/(4.d0*c4)
      wbar=dsqrt(-wbarq)
      cpxx=wbarq*c4/amu(ii)*dlog((amu(ii)+wpi2)/(amu(ii)-wpi2))
      brak1=wpi+cpxx
c
c
      aintxi=0.d0
      ...
      xixi=xi(iii)*xi(iii)
      term1=-wbarq*c4*xixi
      term2=4.d0*ga2/(3.d0*term1)
      term3=(c4+term1)**1.5d0-c4*wpi
c
c
      step47=wxi(iii)*(1.d0-xixi)
     1      *(brak1+term2*term3)
c
 4725 aintxi=aintxi+step47
c
c
 4715 aintmu=aintmu
     1      +aintxi*wmu(ii)*root15/(amu4(ii)*(amu2(ii)+aakk))
c
c
 4705 aa(i)=aa(i)*c5*aintmu
      mi=mi+2
      mm=mm+1
      go to 5999
c
...
c
 8000 return
      ...
      subroutine legp_chinn5 (pj,pjm1,x,j)
c
c
...
c
      ...
      subroutine gset_chinn5(ax,bx,n,z,w)
c
c
...
c
c**********************************************************************
      subroutine dgelg_chinn5(r,a,m,n,eps,ier)
c
c
      implicit real*8 (a-h,o-z)
      dimension a(1),r(1)
...
c
c**********************************************************************
      subroutine dminv_chinn5 (a,n,d,l,m)
      implicit real*8 (a-h,o-z)
...
c**************** this is the end of the program chinn **********************

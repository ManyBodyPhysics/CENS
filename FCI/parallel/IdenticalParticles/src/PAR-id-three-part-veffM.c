/*****************  The module PAR-id-two-part-veffM.c  ******************/
...
// max number of m-scheme matrix element

  num_nondiag_elem  = max_nondiag_id_elem(sp_bas);
  num_nondiag_elem += num_diag_elem;  // an extra ULL_ZERO elem for each (k,l,m) pai
...
static int max_nondiag_id_elem(SP_BAS *sp_bas)
{
  ...
  return number;
     ...
     ** reads a set of uncoupled m-scheme converted two-particle 
     ** matrix plus an additional three-particle matric element
                                          ...
                                          THREE_PART **three_part_ref)
{
  ...
  int         answer, loop, int_data[24], num_elem, new_num_elem;
  int         left, right;
  double      twoElem[2];
  ...
  for(loop = 0; loop < num_elem; loop++) {
    ...
    answer = read_int_number(file_ptr, 24, int_data);
    ...
    // test conservation of total M-value

    left  = int_data[3] + int_data[7] + int_data[11];
    right = int_data[15] + int_data[19] + int_data[23];

    if(left != right) {
      printf("\n\nError in function read_id_three_part_matr_elem():");
      printf("\nWrong M-value for element no %d\n", loop);
      printf("\n leftM_Val = %d  rightM_val = %d",left,right); 
      MPI_Abort(MPI_COMM_WORLD,Rank);
    }
    // end test for total M-value

    // test conservation of total parity

    left  = ((int_data[1] + int_data[5] + int_data[9]) %2);
    right = ((int_data[13] + int_data[17] + int_data[21]) %2);

    if(left != right) {
      printf("\n\nError in function read_id_three_part_matr_elem():");
      printf("\nWrong Parity for element no %d\n", loop);
      printf("\n leftParity = %d  rightParity = %d",left,right); 
      MPI_Abort(MPI_COMM_WORLD,Rank);
    }
    // end test for total parity

      /*
      **       Read effective matrix elements
      ** twoPartElem   = twoElem[0]
      ** threePartElem = twoElem[1]
      */

    if(read_float_number(file_ptr, 2, twoElem) == FALSE) {
      printf("\n\nError in function read_three_part_matr_elem():");
    ...
    threePartPtr->value =  twoElem[0]*(1.0/((double)(model->part - 2)))
                         + twoElem[1];
      /*

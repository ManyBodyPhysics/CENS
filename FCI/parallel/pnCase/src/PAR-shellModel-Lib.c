...
#undef     malloc   
   ...
   if(size == 0) new_memory = NULL_PTR;
	...
	printf("\n\nRank%d: Error from function mem_alloc():",Rank);
...
#undef     calloc   
   ...
   if(num_elem == 0) new_memory = NULL_PTR;
	...
	printf("\n\nRank%d: Error from function mem_calloc():",Rank);
         printf("\nmemory allocation from function %s: %s ", func, var);
...
#undef     realloc   
   ...
   if((new_memory = (void *) realloc(ptr, num_elem * size)) == NULL)  {
     printf("\n\nRank%d: Error from function mem_realloc():",Rank);
      printf("\nmemory allocation from function %s: %s ", func, var); 
      printf("\nMemory request: %d bytes \n", (int)size);
      MPI_Abort(MPI_COMM_WORLD,Rank);
   }
...
TID wallClock(int clock_num, int type)
{
  ...
  if(type == 0) {       // total initialization 
    ...
    printf("\n\nRank%d: Error in function cum_time(): ",Rank);
...
TID cpuClock(int clockNum, int type)
{
  ...
  if(type == 0) {       // total initialization 
    ...
    printf("\n\nRank%d: Error in function cpuClock(): ",Rank);
    printf("\nInput data type = %d is wrong !!\n\n", type);
    MPI_Abort(MPI_COMM_WORLD,Rank);
  }
  return ex_time;
...
TID time_step(int clock_num, int type)
{
  ...
  if(type == 1) { 
    ...
    printf("\n\nRank%d: Error in function time_step(): ",Rank);
    printf("\nInput data type = %d is wrong !!\n\n", type);
    MPI_Abort(MPI_COMM_WORLD,Rank);
  }
  return ex_time;
...
void append_basis_vector_to_file(char *file_name, int n, int dim, double *vector) 
{
  ...
  if((file_ptr = fopen(file_name,(n == 0) ? "wb": "ab+")) == NULL) {
    printf("\n\nRank%d: Error in function append_basis_vector_to_file();",Rank);
  ...
  if(fwrite((const void *)&n,(size_t) sizeof(int), 1, file_ptr) != (size_t)1) {
    printf("\n\nRank%d: Error in function append_basis_vector_to_file()",Rank);
  ...
  if(fwrite((const void *)&dim,(size_t) sizeof(int),1,file_ptr)!= (size_t)1) { 
    printf("\n\nRank%d: Error in function append_basis_vector_to_file()",Rank);
  ...
  if(fwrite((const void *)temp_mem,(size_t) sizeof(float), 
    ...
    printf("\n\nRank%d: Error in function id_write_diag_SD_matr_elem():",Rank);
...
void writeLanczosEigenVec(char *fileName,EIGEN_DATA eigenData,double *vector) 
{
  ...
  if((filePtr = fopen(fileName,(n == 0) ? "wb": "ab+")) == NULL) {
    printf("\n\nRank%d: Error in function writeLanczosEigenVec();",Rank);
    printf("\nWrong file = %s to store eigenVec no %d\n",fileName, n);
    MPI_Abort(MPI_COMM_WORLD,Rank);
  }

  if(fwrite((const void *)&eigenData,(size_t)sizeof(EIGEN_DATA),1,filePtr)!= (size_t) 1) { 
    printf("\n\nRank%d: Error in function  writeLanczosEigenVec()",Rank);
                      ...
                      EIGEN_DATA *eigenData,double *vec_ptr,float *temp_mem)
{
  ...
  if( fread((void *)eigenData,(size_t)sizeof(EIGEN_DATA),1,file_ptr) != (size_t)1)  {
    printf("\n\nRank%d: Error in function  readLanczosEigenVecFromOpenFile()",Rank);
           ...
           EIGEN_DATA *eigenData, double *vec_ptr,float *temp_mem)
{
 ...
 if((filePtr= fopen(filename,"rb")) == NULL)   {
   printf("\n\nRank%d: Error in function readShellmodelEigenVec()",Rank);
  ...
  if(fread((void *)eigenData,(size_t)sizeof(EIGEN_DATA),1,filePtr) != (size_t)1)  {
    printf("\n\nRank%d: Error in function readShellmodelEigenVec()",Rank);
    printf("\nIn reading struct eigenDatafrom file %s\n",filename);
    MPI_Abort(MPI_COMM_WORLD,Rank);
  } // end of if-test
                      ...
                      int n, int dim, double *vec_ptr, float *temp_mem)
{
  ...
  if( fread((void *)&vec_no,(size_t) sizeof(int), 1, file_ptr) != (size_t)1)  {
    printf("\n\nRank%d: Error in function read_basis_vector_from_open__file()",Rank);
   ...
   if( fread((void *)&num_elem,(size_t) sizeof(int),1,file_ptr)!= (size_t)1) {
     printf("\n\nRank%d: Error in function read_basis_vector_from_open__file()",Rank);

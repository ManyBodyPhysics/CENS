...
void id_slater_determinant(SHELL *model, SP_BAS *sp_bas, SD_BAS *sd_bas)
{
  ...
  { // local block  to print to output file: number of |SD>
    ...
    if(sd_bas->MJ == 0)  {
      fprintf(file_ptr,"\n\nNumber of asymmetric slater determinants =%8d", sd_bas->numSD[0]);
      fprintf(file_ptr,"\nNumber of symmetric slater determinants  =%8d", sd_bas->numSD[1]);
      fprintf(file_ptr,"\nTotal number of slater determinants      =%8d",
	      2 * sd_bas->numSD[0] + sd_bas->numSD[1]);
    }
    else {
      fprintf(file_ptr,"\n\nNumber of slater determinants = %d", sd_bas->numSD[0]);
    }
    fclose(file_ptr);

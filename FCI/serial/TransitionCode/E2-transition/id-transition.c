...
void id_particle_transitions(int trans_type)
{
  ...
  j_trans = 0;       /* initialization */
   ...
   for(trans = 0,init_save = -1; trans < Model[0].num_trans; trans ++) {
	...
	readEigenDataFile(File_eigen_init, init_num,
                  Info_id[0].num[0] + Info_id[0].num[1],
                          Model[0].numj_N, Eigen_vec);
	...
	readEigenDataFile(File_eigen_final, final_num,
                  Info_id[0].num[0] +Info_id[0].num[1],
                          Model[0].numj_N, Eigen_vec);
...
void readEigenDataFile(char *filename, int eigenVecNum, int dim, 
                                 int numj_occ, double *vecPtr)
{
  int             k;
  double          *j_occ, *finalPtr;
  float           *initPtr, *floatMem;
  EIGEN_DATA      h_eigenData;
  FILE            *filePtr;

  if((filePtr = fopen(filename,"rb")) == NULL)  { 
    printf("\n\nError in function readEigenDataFilereadEigenData():");
    printf("\nNot allowed to open file %s\n", filename);
    exit(1);
  }
  fseek(filePtr,(long)(eigenVecNum*(dim*sizeof(float)
			  + sizeof(EIGEN_DATA)
  ...
  if(fread((void *)&h_eigenData,(size_t) sizeof(EIGEN_DATA), 1, 
                                                 filePtr) != 1) {
    printf("\nError in function readEigenDataFile()");
  ...
  if(h_eigenData.vecNum != eigenVecNum) {
    printf("\nError in function rreadEigenDataFile()");
  ...
  floatMem = (float *)malloc(dim * sizeof(double));
